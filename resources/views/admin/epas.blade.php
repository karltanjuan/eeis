        ...
        <div class="col-md-9">
            <div class="card">
                <div class="card-header">
                    <a href="{{url('/admin')}}">Admin Dashboard</a> / EPAS Inventory
                </div>
                <div class="card-body">
                  <div class="row">
                        <div class="col-md-3 mb-3">
                            <a href="{{url('admin/epas/create')}}" class="btn btn-primary" id="add_item">Add New Item</a>
                        </div>
                        <div class="col-md-9">
                            <span class="float-right">
                                <b>Electronics Product Assembly and Servicing</b>
                            </span>
                        </div>
                  </div>
                  <div class="row">
                      <div class="col-md-12">
                          ...
                          <table class="table table table-hover table-sm epas-table">
                                <thead>
                                    <tr>
                                    ...
                                    @if (count($items) > 0)
                                        @foreach ($items as $item)
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                  <a href="{{url('admin/epas/')}}/{{$item->id}}/edit" class="btn btn-sm btn-outline-primary">
...
<script>
    ...
    //         _token: "{{ csrf_token() }}",
    //         'search': $('#input_search').val(),
    //         'type': $('#filter_type option:selected').val(),
        ...
        .then(function (response) {
            ...
            if (response.data.items.length > 0) {
                $.each(response.data.items, function(x,y) {
                    ...
                    html += "</td>"
                    html += "<td>"
                    html += "<a href='{{url('admin/epas/edit/')}}/"+y.id+"' class='btn btn-sm btn-outline-primary'>"
            ...
            // $(".epas-table").dataTable().clear().destroy()
            // get_list()
            // $('.epas-table').dataTable({
            //     'order': [[5, 'desc']]
            // })
